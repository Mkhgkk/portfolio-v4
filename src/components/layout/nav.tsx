...
export function Nav() {
  ...
  return (
    <header className="fixed top-4 left-1/2 -translate-x-1/2 z-50 w-full max-w-2xl px-4">
      <nav className="flex items-center justify-between px-3 py-2 rounded-full border border-neutral-200 dark:border-neutral-800 bg-neutral-100/80 dark:bg-neutral-950/80 backdrop-blur-sm">
        <div className="flex items-center">
          {navItems.map((item) => (
            <Link
              ...
              href={item.href}
              className={cn(
                "px-2.5 py-1 sm:px-3 sm:py-1.5 rounded-full transition-colors",
                active === item.id
            ...
            >
              <span className="text-xs sm:hidden">{item.short}</span>
